...
const GestureInput = () => {
    ...
    const gestureStreak = useRef({ name: null, count: 0, lastStable: null });
    ...
    // 记录上一帧手掌中心位置，用于计算位移差
    const lastPalmPos = useRef(null);
    // 记录上一帧双手距离，用于缩放
    const lastHandDistance = useRef(null);
    // 记录上一帧单手尺寸，用于单手缩放
    const lastHandScale = useRef(null);

    const isExtended = (landmarks, tipIdx, mcpIdx, wrist) => {
